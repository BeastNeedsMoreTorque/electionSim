...
trait VoteXfer {
  ...
  def xferTable: TypedTag[String] = {
    ...
    val headers =
      tr(th(left)("Xfer from", raw("&darr;"), " to", raw("&rarr;")), for (p1 ← Party.values.sortBy(_.entryName)) yield th(right)(p1.toString))

    table(cls := "voteTransfers")(
      headers,
      for (p1 ← Party.values.sortBy(_.entryName)) yield tr(
        td(left)(p1.toString),
        for {
          p2 ← Party.values.sortBy(_.entryName)
          pct = (xfer(p1, p2) * 100).toInt
        } yield td(right)(if (pct > 0) pct.toString else "")
      )
    )
  }
