...
/** Created by bwbecker on 2016-10-01.
  */
case class ElectionResults(val elected: Vector[Candidate], val unelected: Vector[Candidate], val topup: Vector[Candidate], design: Design) {

  val allRidingCandidates = elected ++ unelected

  val allCandidates = allRidingCandidates ++ topup

  val electedByRegion: Map[RegionId, Vector[Candidate]] = (elected ++ topup).groupBy(cand ⇒ cand.regionId)

  val electedByRiding: Map[RidingId, Vector[Candidate]] = elected.groupBy(cand ⇒ cand.ridingId)

  val candidatesByRiding: Map[RidingId, Vector[Candidate]] = allCandidates.groupBy(cand ⇒ cand.ridingId)

  val candidatesByRegion: Map[RegionId, Vector[Candidate]] = allCandidates.groupBy(cand ⇒ cand.regionId)

  def topupByRegion(regionId: RegionId): Vector[Candidate] = electedByRegion(regionId).filter(c ⇒
    c.seatType ==
      SeatType.TopupSeat
  )
  ...
  /** Proportionality analysis for candidates elected in a set of ridings with an optional set of top-up seats.
    */
  ...
  def compositeGallagher: Double = {
    ...
    val wtGallagher =
      aLst.foldLeft(0.0)((accum, a) ⇒
        accum + a.gallagherIndex * a.totalSeats / totSeats
      ) // aLst.map(a ⇒ a.gallagherIndex * a.totalSeats / totSeats).sum
  ...
  def analysisByRegion(regions: Seq[Region]): Analysis =
    new Analysis(regions.flatMap(region ⇒ candidatesByRegion(region.regionId)), regions.map(r ⇒ r.totalCandidates).sum)
