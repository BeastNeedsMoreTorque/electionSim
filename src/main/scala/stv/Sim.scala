...
case class Sim( // fptpRidings: Map[RidingId, Riding],
  ...
  /** A List[(cumPop:Double, area:Int)] where cumPop percentage of the population lives in a riding no bigger than area km2.
    */
  ...
  /** How many adjustment seats are in an average region? Hmmm... This depends on the election algorithm, which we don't have....
    */
  ...
  /** Get the maximum riding area for a given percentage of the population. For maxRidingAreaForPctOfPopulation(0.50), 50% of Canada's population
  ...
  /** Average number of MPs per riding. Exclude Territories and PEI.
    */
  ...
  def sensitivityAnalysis(party1: Party, party2: Party): Vector[SensitivityDataPoint] = {
    ...
    val lst = (for (d ← SENSITIVITY_START until SENSITIVITY_END by SENSITIVITY_STEPS) yield {
      ...
      SensitivityDataPoint(d, libVotes, libMPs, conVotes, conMPs, ndpVotes, ndpMPs, grnVotes, grnMPs, blocVotes, blocMPs, results.compositeGallagher)
    }).toVector
