...
class RcStvElectionStrategy(val voteXfer: VoteXfer) extends RidingElectionStrategy {
  ...
  /** Return (a, b, c) where a is the list of elected candidates, b is the list of remaining unelected candidates, and c is the id of the old riding
    * which becomes the adjustment riding.
    */
          ...
          case Some(cut) ⇒
            ...
            if (debug && remaining.length < dm + 3)
              println("  Remaining = " + remaining.map(c ⇒
                f"${c.name.take(18)}%-20s ${c.effVotes}%7.1f"
              ).mkString("\n\t"))
  ...
  /** Find the candidate with the fewest effective votes that still has someone else from their local riding that's in the race.
    */
  def candidateToCut(remaining: Seq[Candidate], protectLast: Boolean): Option[Candidate] = {
    def helper(c: Seq[Candidate]): Option[Candidate] =
      c match {
        ...
        case head +: tail
            if tail.find(c ⇒
              c.oldRidingId == head.oldRidingId
            ).isDefined ⇒ Some(head)
  ...
  /** Transfer votes from the cut candidate to other candidates. For now, just distribute evenly to all the remaining candidates in the given party.
    * Later might consider taking into account local riding effects.
    */
...
object RcStvProvAdjustment {
  ...
  def adjustmentMPs(prov: Province, alreadyElected: Vector[Candidate], notYetElected: Vector[Candidate]): Vector[Candidate] = {
    /*
    ...
    def helper(adjustMPs: Set[Candidate], notElected: Set[Candidate], oldRidingsWithNoMP: Set[Int]): Vector[Candidate] = {
      ...
      if (adjustMPs.size == mpsNeeded)
        ...
        helper(adjustMPs + cand.copy(winner = true, seatType = SeatType.AdjustmentSeat), notElected - cand, oldRidingsWithNoMP - cand.oldRidingId)
      }
    }
