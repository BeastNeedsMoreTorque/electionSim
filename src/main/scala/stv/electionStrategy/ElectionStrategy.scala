...
trait RidingElectionStrategy extends ElectionStrategy {
  ...
  def sortCandidates(candidates: Seq[Candidate]): Seq[Candidate] =
    candidates.sortBy(c ⇒ (-c.order, -c.votes))

  def runElection(candidates: Seq[Candidate], dm: Int): (Seq[Candidate], Seq[Candidate])

  /** Return the same list but with the effective votes either the total of all votes for the party (assigned to the candidate with the most votes) or
    * 0.
    */
  def collapseCandidates(candidates: Seq[Candidate]): Seq[Candidate] = {
    val byParty        = candidates.groupBy(_.party)
    val winnersByParty = byParty.map(t ⇒ t._1 → t._2.maxBy(_.votes))
    for (c ← candidates)
      yield
...
trait TopupElectionStrategy extends ElectionStrategy {
  ...
  def runElection(regionId: String, allCandidates: Vector[Candidate], numSeats: Int, threshhold: Double): Vector[Candidate]
}
...
object TopupStrategy extends TopupElectionStrategy {
  ...
  def runElection(regionId: String, allCandidates: Vector[Candidate], numTopupSeats: Int, threshhold: Double): Vector[Candidate] = {
    ...
    def helper(topups: Vector[Candidate]): Vector[Candidate] =
      if (topups.length == numTopupSeats)
        ...
        val a =
          new Analysis(allCandidates ++ topups, numRidingSeats + numTopupSeats)
...
object NotApplicableTopupElectionStrategy extends TopupElectionStrategy {
  ...
  val description: TypedTag[String] = p(
...
object FptpRidingElectionStrategy extends RidingElectionStrategy {
  ...
  override def sortCandidates(candidates: Seq[Candidate]): Seq[Candidate] =
    candidates.sortBy(c ⇒ (-c.votes, c.name))
...
object NotApplicableRidingElectionStrategy extends RidingElectionStrategy {
  ...
  val help         = p("No election strategy is applicable in this situation.")
  val description: TypedTag[String] = p(
    "An election strategy for where none are applicable.  For example, for " +
      "multi-member" +
