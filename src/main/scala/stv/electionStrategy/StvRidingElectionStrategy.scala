...
/** Just like STV but we want the AV name.
  */
object EkosAvRidingElectionStrategy
    extends StvRidingElectionStrategy(EkosXfer) {
  override val name      = "AV-Ekos"
  override val shortName = "A"
  override val help = p(
    "Alternative Vote: Elect one winner using a ranked ballot.  Drop the least " +
...
object ThinAirAvRidingElectionStrategy
    extends StvRidingElectionStrategy(ThinAirXfer) {
...
object EkosStvRidingElectionStrategy
    extends StvRidingElectionStrategy(EkosXfer) {
  override val name      = "STV-Ekos"
  override val shortName = "S"
}

object ThinAirStvRidingElectionStrategy
    extends StvRidingElectionStrategy(ThinAirXfer) {
  override val name      = "STV-ThinAir"
  override val shortName = "T"
}

class StvRidingElectionStrategy(val voteXfer: VoteXfer)
    extends RidingElectionStrategy {

  val name      = "STV"
  val shortName = "S"
  val help = div(
    p(
      "Single Transferable Vote: Elect one or more winners using a ranked ballot. Establish a 'quota' or" +
        " " +
        "maximum number of votes required to be elected.  Call that number 'Q'."
    ),
    p(
      "If a candidate has more than Q votes, declare the candidate elected and transfer any votes more than " +
  ...
  val description =
    div(
      p("""During the election in each riding, votes were transferred in two
      ...
      p("""When the last member of a party is eliminated, the votes are transferred
  ...
  def runElection(candidates: Seq[Candidate], dm: Int): (Seq[Candidate], Seq[Candidate]) =
    this.stvElection(
      candidates,
      dm,
      (p1: Party, p2: Party, dm: Int) => voteXfer.xfer(p1, p2),
      debug
    )

  private def stvElection(candidates: Seq[Candidate], dm: Int, xferVoteProb: VoteXferFunc, debug: Boolean): (Seq[Candidate], Seq[Candidate]) = {
    def c2m(c: Candidate): MutCandidate =
      MutCandidate(
        c.ridingId,
        c.oldRidingId,
        c.regionId,
    ...
    def m2c(m: MutCandidate): Candidate =
      Candidate(
        m.ridingId,
        m.oldRidingId,
        m.regionId,
    ...
    val (win, lose) =
      stvElection(candidates.map(c2m(_)).toSet, dm, xferVoteProb)(debug)
  ...
  /** Count the votes for this election.
    */
  private def stvElection(candidates: Set[MutCandidate], dm: Int, xferVoteProb: VoteXferFunc)(
    implicit debug: Boolean = false
...
$totalVotes, thr = $threshhold, dm = $dm""")
        ...
        candidates.find(_.effVotes >= threshhold) match {
          ...
          case None ⇒
            if (elected.length + candidates.size == dm) {
              dp(
                s"Exactly enough candidates left to finish the election.  $candidates"
              )
              candidates.foreach { c ⇒
                c.winner = true; c.order = order
              }
    ...
    dp(s"losers = $losers")
    assert(
      winners.length + losers.length == candidates.size,
      s"${winners.length} + ${losers.length} != ${candidates.size}"
    )
    (winners, losers)
  }
  ...
  private def distributeVotes(
    votes: Double,
    party: Party,
    remaining: Set[MutCandidate],
    dm: Int,
    xferVoteProb: VoteXferFunc
  )(implicit debug: Boolean = false): Set[MutCandidate] = {
    // assert(remaining.size >= 1)
    val (sameParty, otherParties) = remaining.partition(_.party == party)
    if (sameParty.size > 0) {
      dp(s"transferring $votes votes from $party to ${sameParty.map {
          _.party
        }}.")
      // transfer votes to other members of the same party
      val xfer = votes / sameParty.size
      val r = sameParty.map { c ⇒
        c.effVotes = c.effVotes + xfer; c
      } ++ otherParties
      assert(r.size == remaining.size)
      r
    }
    else {
      dp(s"transferring $votes votes from $party to ${otherParties.map {
          _.party
        }}.")
  ...
  private case class MutCandidate(
    ridingId: RidingId,
    oldRidingId: Int,
    regionId: RegionId,
    provName: ProvName,
    ...
    override def toString =
      s"Candidate($ridingId, $name, $party, $votes, $effVotes)"
