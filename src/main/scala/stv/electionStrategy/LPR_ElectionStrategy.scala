...
class LPR_ElectionStrategy(val voteXfer: VoteXfer) extends RidingElectionStrategy {
  ...
  /** Choose a candidate to cut -- the one with the fewest effective votes that isn't protected. Return the remaining hopeful candidates. Anyone found
    * in need of protection is protected.
    */
  ...
  /** Transfer votes from a losing candidate to the hopeful candidates. Votes are transferred at full value according to the vote transfer function.
    */
  ...
  /** Calculate who wins and who loses.
    *
    * @param orig_candidates
    *   The candidates vying for the seats
    * @param dm
    *   The number of seats available
    * @return
    *   (elected candidates, not-elected candidates)
    */
  def runElection(orig_candidates: Seq[Candidate], dm: Int): (Seq[Candidate], Seq[Candidate]) = {
    ...
    def handleNewWinner(newWinner: Cand, winners: CandSeq, hopeful: CandSeq, losers: CandSeq): (CandSeq, CandSeq) = {
      ...
      helper(
        newWinner.withVotes(quota).withOrder(this.nextOrder) +: winners,
    ...
    def helper(winners: CandSeq, hopeful: CandSeq, losers: CandSeq): (CandSeq, CandSeq) = {
  ...
  case class Cand(candidate: Candidate, protect: Boolean = false) {
