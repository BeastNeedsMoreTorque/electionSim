...
case class RegionResultsHTML(params: Params, sim: Sim) extends Page {
  ...
  protected def content: TypedTag[String] =
    div(
      descr,
      links,
      for { region ← sim.regions.sortBy(_.regionId) } yield doRegion(region)
    )
  ...
  private def links: TypedTag[String] =
    p(cls := "links")(
      for (region ← sim.regions.sortBy(_.regionId)) yield span(a(href := s"#${region.regionId}")(region.regionId), " ")
    )
        ...
        table()(
          tr(
            td(cls := "topup")(
              ...
              ol(
                for (c ← sim.results.topupByRegion(region.regionId)) yield li(cls := c.party.toString)(c.party.toString)
              )
            ),
  ...
  def constituentRidings(targetRegion: String): Vector[String] = {
    ...
    } yield newRiding.mapping.map(or => or.name)).flatten
