...
case class ParametersHTML(params: Params, sim: Sim) extends Page {
  ...
  protected val content = div(
    ...
    ul(
      li(
        a(href := s"https://github.com/bwbecker/electionSim/blob/master/json/candidates.json")("candidates.json"),
        ": Candidates from an actual election."
      ),
      li(
        a(href := s"https://github.com/bwbecker/electionSim/blob/master/json/ridings.json")("ridings.json"),
        ": Information about each real riding (riding id, name, population, area)."
      ),
      li(
        a(href := s"https://github.com/bwbecker/electionSim/blob/master/json/${params.designName}.json")(
          s"${params.designName}.json"
        ),
        ": The riding structure used for this simulation -- mapping FPTP " +
        ...
        table(
          ...
          tbody(
            ...
            params.voteAdjustment.map {
              va ⇒ tr(td("Vote adjustment: "), td(s"${va.toParty}.votes += ${va.pct} * ${va.fromParty}.votes"))
            },
