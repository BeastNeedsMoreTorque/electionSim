...
case class RidingResultsHTML(params: Params, sim: Sim) extends Page {
  ...
  protected def content: TypedTag[String] =
    div(cls := "ridingResults")(
      ...
      disclaimer,
      links,
      for (prov <- sim.design.provinces) yield doProvince(prov)
    )
  ...
  private def links: TypedTag[String] =
    p(cls := "links")(
      for (provName ← ProvName.values) yield span(a(href := s"#$provName")(provName.entryName), " ")
    )

  def doProvince(prov: Province) =
    div(
      h2(id := prov.prov.entryName)(prov.prov.longName),
      for (region ← prov.regions) yield div(
        h3(s"Region: ${
            region.regionId
          }"),
        table(cls := "ridingSummary")(
          tr(th("Riding ID"), th("Composed from"), th("Candidate"), th("Party"), th("Votes"), th("Eff Votes")),
          for (riding <- region.ridings) yield doRiding(riding)
        )
      )
    )
