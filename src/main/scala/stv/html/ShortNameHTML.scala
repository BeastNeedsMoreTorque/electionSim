...
object ShortNameHTML extends Page {
  ...
  protected def content: TypedTag[String] =
    div(
      ...
      ul(
        for (dn <- DesignName.values) yield li(s"${dn.shortName}: ${dn.descr}")
      ),
      p("The third letter is the election strategy used in single-member ridings.  They are:"),
      ul(
        for (sms <- RidingElectionStrategy.singleMbrStrategies) yield li(s"${sms.shortName}: ", sms.help)
      ),
      p("The fourth letter is the election strategy used in multi-member ridings.  They are:"),
      ul(
        for (mms <- RidingElectionStrategy.multiMbrStrategies :+ NotApplicableRidingElectionStrategy) yield li(s"${mms.shortName}: ", mms.help)
      )
    )
}
