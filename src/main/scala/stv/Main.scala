...
object Main {
  ...
  val clArgParser = new scopt.OptionParser[CLArgs]("scopt") {
    ...
    opt[Unit]("all").action((_, c) =>
      c.copy(all = true, years = numRidingsByElectionYr.keys.toVector.sorted, designs = DesignName.values, overview = true, voteSwing = true)
    ).text("Produce all possible combinations")

    opt[Unit]("overview").action((_, c) =>
      c.copy(overview = true)
    ).text("Write the overview pages")

    opt[Unit]("voteSwing").action((_, c) =>
      c.copy(voteSwing = true)
    ).text("Perform vote swing analysis")

    opt[Seq[Int]]("years").valueName(numRidingsByElectionYr.keys.toVector.sorted.mkString("<", ">,<", ">")).action((x, c) =>
      c.copy(years = x)
    ).text("election years to base simulations on")

    opt[Seq[DesignName]]("designs").valueName(DesignName.values.mkString("<", ">,<", ">")).action((x, c) =>
      c.copy(designs = x)
    ).text("designs for the simulations")
  ...
  val fptpDescr = div(
    p(
      a(href := "https://en.wikipedia.org/wiki/First-past-the-post_voting")("First-Past-The-Post"),
  ...
  val avDescr = div(
    p(
      a(href := "https://en.wikipedia.org/wiki/Instant-runoff_voting")("Alternative Vote"),
  ...
  val stvMedDescr = div(
    ...
    p(
      ...
      """, President of """,
      a(href := "https://fairvotingbc.com/")("Fair Voting BC"),
      "."
    )
  )
  ...
  val mmp8avDescr = div(
    p(
      "A ",
      a(href := "https://en.wikipedia.org/wiki/Mixed-member_proportional_representation")("Mixed Member Proportional"),
  ...
  val avPlusDescr = div(
    p(
      """This design assumes that we keep our current 338 single-member ridings but
  ...
  val rup338Descr =
    div(
      p(
        "The ",
        a(href := "../RuralUrbanProportional_20160706.pdf")("Rural-Urban Proportional"),
  ...
  val ridingCentricDescr = div(
    p(
      strong("""Riding-Centric Rural Urban Proportional"""),
      """ (need a better name!) is an exciting new electoral
    systems
    model that builds on the Rural-Urban model with ideas from """,
      a(href := "http://www.parl.gc.ca/Content/HOC/Committee/421/ERRE/Brief/BR8457966/br-external/ElbertLeonid-e.pdf")("Local Transferable Vote"),
    ...
    ul(
      li(
        strong("""Excellent proportionality"""),
        """:  It has a composite Gallagher score of 2.2.  Out of the
      roughly 50 models
      ...
      li(
        strong("""An elected MP in every current riding"""),
        """:  This model keeps our current, 2015, ridings
      ...
      li(
        strong("""Multi-member ridings"""),
        """:  Electing multiple MPs to serve a geographical area helps ensure that
      ...
      li(
        strong("""Easy redistricting"""),
        """:  Many models require Elections Canada to redraw riding boundaries.
      This one
      ...
      li(
        strong("Few compensatory seats"),
        """: This model has only 42 compensatory seats, or about 12%.  MMP,
    ...
    ul(
      li(
        """Add 42 compensatory seats.  The existing seat proportions among the provinces would be almost
      ...
      li(
        """Put multi-member ridings into 3 regions in Ontario and 2 regions in Quebec.  Everywhere else
    ...
    p(
      """There is one down side to Riding-Centric Rural-Urban:  the 42 additional compensatory seats
  ...
  val featuredSystems = List(
    Params("fptp", 2015, "First-Past-The-Post", DesignName.fptp, s"fptp", Some(fptpDescr), MMP_FPTP),
    Params("av", 2015, "Alternative Vote", DesignName.fptp, s"av", Some(avDescr), MMP_AV),
    Params("stv_med", 2015, "Single Transferable Vote (Medium-sized Regions)", DesignName.stv_med, s"stv_med", Some(stvMedDescr), STV),
    Params("stv_small", 2015, "Single Transferable Vote (Small Regions)", DesignName.stv_small, s"stv_small", Some(stvSmallDescr), STV),
    Params("mmp-8-fptp", 2015, "Mixed Member Proportional (Small Regions)", DesignName.mmp_small, s"mmp-8-fptp", Some(mmp8fptpDescr), MMP_FPTP),
    Params("mmpLite", 2015, "Mixed Member Proportional (Lite)", DesignName.mmp_enlargeP, s"mmp-15pct", Some(mmpLiteDescr), MMP_FPTP),
    Params("av+", 2015, "AV+", DesignName.mmp_enlargeP, s"av-plus", Some(avPlusDescr), MMP_AV),
    Params(
      "rup-338",
      2015,
      "Rural-Urban PR (More Singles, 338 Seats)",
      DesignName.ru_singles,
      s"rup-338",
      Some(rup338Descr),
    ...
    Params(
      "rup-15pct",
      2015,
      "Rural-Urban PR (More Singles, 389 Seats)",
      DesignName.ru_enlargeP,
      s"rup-15pct",
      Some(rup15PctDescr),
    ...
    Params(
      "rup-stv",
      2015,
      "Rural-Urban PR (Few Singles)",
      DesignName.ru_multiples,
      s"rup-stv",
      Some(stvPlusDescr),
      STVplus
    ...
    Params(
      "ru_multiples_rc2",
      2015,
      "Riding Centric Rural-Urban PR",
      DesignName.ru_multiples_rc2,
      s"ru_multiples_rc2",
      Some(ridingCentricDescr),
      RcRUPR2
    ),
    Params("lpr_no_topup", 2015, "Local Proportional Representation (no top-up)", DesignName.lpr_no_topup, s"LPR_no_topup", None, LPR),
    Params(
      "lpr_with_topups",
      2015,
      "Local Proportional Representation (with top-ups)",
      DesignName.lpr_with_topups,
      s"LPR_with_topups",
      None,
      ToppedUpLPR
    )
  ...
  val variantSystems = List(
    Params(
      "mmp-8-av",
      2015,
      "MMP (Small Regions, STVplus)",
      DesignName.mmp_small,
      s"mmp-8-av",
      Some(mmp8avDescr),
      STVplus
      // EkosAvRidingElectionStrategy, EkosStvRidingElectionStrategy
    ...
    Params(
      "rup-338-list",
      2015,
      "Rural-Urban PR (More Singles, 338 Seats, ListPR)",
      DesignName.ru_singles,
      s"rup-338-list",
      Some(rup338ListDescr),
    ...
    Params(
      "rup-15pct-stv",
      2015,
      "Rural-Urban PR (More Singles, More Seats)",
      DesignName.ru_enlargeP,
      s"rup-15pct-stv",
      Some(rup15PctDescr),
      STVplus
...
/** Adjust the votes for sensitivity analysis. toParty.votes += pct * fromParty.votes
  */
case class VoteSwing(pct: Double, fromParty: Party, toParty: Party)

case class Params(
  name: String, // identify this set of parameters
  year: Int,
  ...
  def matches(p: Params): Boolean = this.matches(p.designName, p.year, p.electionStrat.sm, p.electionStrat.mm)
